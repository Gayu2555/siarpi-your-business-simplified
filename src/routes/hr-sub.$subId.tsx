...
import { getHrSubModule, hrSubModules, type HrSubModuleDetail } from "@/lib/modules/hr";
import { getModuleIcon } from "@/lib/modules";
import { ArrowLeft, ArrowRight, Star, Check, ShieldCheck, Zap, Layers, Sparkles, HelpCircle } from "lucide-react";
...
export const Route = createFileRoute("/hr-sub/$subId")({
  head: ({ params }) => {
    const d = getHrSubModule(params.subId);
    if (!d) {
      return { meta: [{ title: "Sub-Modul HR Tidak Ditemukan | Siarpi ERP" }] };
    }

    const metaTitle = `Software ${d.name} HR Terbaik Indonesia | ${d.tagline} | Siarpi ERP`;
    const metaDesc = `Optimalkan ${d.name.toLowerCase()} di Siarpi ERP: ${d.tagline}. ${d.longDescription.slice(0, 150)}. Coba gratis 14 hari tanpa kartu kredit!`;
    const canonicalUrl = `https://siarpi.com/hr-sub/${d.id}`;
    const keywords = [
      `software hr indonesia`,
      `aplikasi ${d.name.toLowerCase()} bisnis`,
      `software sdm indonesia`,
  ...
  notFoundComponent: () => (
    <div className="flex min-h-screen flex-col bg-background">
      ...
      <main className="container mx-auto flex flex-1 flex-col items-center justify-center px-4 py-20 text-center">
        <h1 className="font-display text-4xl font-bold">Sub-Modul HR Tidak Ditemukan</h1>
        ...
        <Button asChild className="mt-8 bg-gradient-primary text-primary-foreground">
          <Link to="/hr">Kembali ke Modul HR</Link>
        </Button>
      </main>
      <Footer />
    </div>
  ...
  component: HrSubModulePage,
});

function HrSubModulePage() {
  const { detail: d } = Route.useLoaderData() as { detail: HrSubModuleDetail };
  const SubIcon = getModuleIcon(d.iconName || "UserCheck");

  const related = hrSubModules
    ...
    <div className="flex min-h-screen flex-col bg-background">
      ...
      <main className="flex-1">
              ...
              <div>
                ...
                <div className="mt-6 flex flex-wrap gap-3">
                  ...
                  <div className="flex items-center gap-2 rounded-xl bg-card border border-border/80 px-3.5 py-2 text-xs font-semibold shadow-2xs">
                    ...
                    <span>Otomatisasi Real-time</span>
                  </div>
                  <div className="flex items-center gap-2 rounded-xl bg-card border border-border/80 px-3.5 py-2 text-xs font-semibold shadow-2xs">
                    <Layers className="h-4 w-4 text-sky-500" />
                    <span>Akses Berjenjang</span>
                  </div>
                </div>
                ...
                <div className="mt-8 flex flex-col sm:flex-row items-center gap-4">
                  ...
                  <Button size="lg" variant="outline" asChild className="w-full sm:w-auto">
                    <Link to="/komparasi">Pelajari Integrasi ERP</Link>
                  </Button>
                </div>
              </div>
                  ...
                  <div className="flex items-center justify-between border-b border-border/60 pb-5">
                    <div className="flex items-center gap-3.5">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-muted-foreground">Siap Digunakan • Sub-Modul HR</p>
                      </div>
                    </div>
        ...
        <section className="container mx-auto px-4 py-16 md:px-6 md:py-24">
          <div className="mx-auto max-w-3xl text-center mb-12">
            ...
            <p className="mt-3 text-base text-muted-foreground">
              Manfaat utama yang langsung dirasakan oleh tim HR, manajemen, dan seluruh karyawan perusahaan.
            </p>
          </div>
        ...
        <section className="border-t border-border bg-muted/20 py-16 md:py-20">
          <div className="container mx-auto px-4 md:px-6">
            <h3 className="font-display text-2xl font-bold mb-8 text-foreground">Sub-Modul HR Lainnya</h3>
